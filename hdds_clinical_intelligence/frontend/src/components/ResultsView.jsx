/*
  ...
  Shows patient switcher, KPIs, and detailed agent outputs.
*/
import { useState } from 'react';
import {
  User, ShieldAlert, AlertTriangle,
  CheckCircle, Clock, FileText, TrendingUp, ChevronDown, ChevronUp,
  MessageSquare, X, Send, Loader2, Download, Plus
} from 'lucide-react';
...
function Section({ title, icon, children, defaultOpen = true }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm result-card">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-gray-50 rounded-t-xl"
      >
        <div className="flex items-center gap-2">
          {icon}
          <h3 className="font-semibold text-ey-dark">{title}</h3>
        </div>
...
export default function ResultsView({ data, onBack, token, role }) {
  ...
  const visiblePatients = data.patients.filter((p) => {
    if (riskFilter === 'All') return true;
    return p.agent_results.risk_assessment.risk_level === riskFilter;
  });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 bg-ey-yellow text-ey-dark rounded-lg hover:bg-[#e6cf00] transition-colors"
        >
          <Plus className="w-3.5 h-3.5" /> New Analysis
        </button>
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {role === 'admin' && (
            <button className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-lg transition-colors">
              <ShieldAlert className="w-3.5 h-3.5" /> Admin Settings
            </button>
          )}
          <button
            onClick={handleDownloadFHIR}
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg transition-colors"
          >
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-3 sm:p-4 mb-6">
          <div className="flex items-center justify-between gap-3 mb-3">
            <h3 className="font-semibold text-gray-700 flex items-center gap-2 text-sm">
              <User className="w-4 h-4" /> Patients ({visiblePatients.length})
            </h3>
            <select
              value={riskFilter}
              onChange={(e) => setRiskFilter(e.target.value)}
              className="border border-gray-300 rounded-lg px-2 py-1.5 text-xs font-medium focus:outline-none focus:border-ey-yellow"
            >
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
            {data.patients.map((p, i) => {
              ...
              const hidden = riskFilter !== 'All' && pRisk !== riskFilter;
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 text-left px-3.5 py-2 rounded-lg border transition-all whitespace-nowrap ${
                    active
                      ? 'bg-ey-yellow border-[#e6cf00] text-ey-dark shadow-sm'
                      : 'bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100'
                  }`}
                >
                  <div className="font-medium text-sm">{p.patient_name}</div>
                  <div className={`text-[10px] font-bold mt-0.5 uppercase ${severityColor(pRisk === 'High' ? 'High' : pRisk === 'Medium' ? 'Moderate' : 'Low')}`}>
                    {pRisk} Risk
                  </div>
                </button>
              );
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 result-card">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 result-card">
          ...
          <p className="text-2xl font-bold text-ey-dark">{ar.early_detection.total_flags || 0}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 result-card">
          <p className="text-xs text-gray-400 uppercase font-medium mb-1">Recommendations</p>
          <p className="text-2xl font-bold text-ey-dark">{ar.recommendations.total_recommendations || 0}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 result-card">
      ...
      <div className="space-y-4">
        ...
        <Section title="Clinical Summary" icon={<FileText className="w-4 h-4 text-blue-500" />}>
          <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3 mb-4">
            <p className="text-xs text-blue-800">
              <strong>What is this?</strong> This is an AI-generated synthesis of the patient's current medical state. It combines all unstructured documents, conditions, and active medications into a single cohesive narrative for quick review.
            </p>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {ar.clinical_summary.active_conditions.map((c, i) => (
                  <div key={i} className="text-xs bg-gray-50 border border-gray-100 p-2 rounded-lg flex justify-between items-center group">
                    <div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {ar.clinical_summary.current_medications.map((m, i) => (
                  <div key={i} className="text-xs bg-blue-50 border border-blue-100 p-2 rounded-lg flex justify-between items-center group">
                    <div>
        ...
        <Section title="Risk Assessment" icon={<ShieldAlert className="w-4 h-4 text-red-500" />}>
          <div className="flex items-center gap-4 mb-4 flex-wrap">
            <div className={`text-3xl font-extrabold px-5 py-2 rounded-xl border ${riskStyles(ar.risk_assessment.risk_level)}`}>
              {ar.risk_assessment.risk_level}
            </div>
            <div>
          ...
          {ar.risk_assessment.rationale && (
            <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">{ar.risk_assessment.rationale}</p>
          )}
        </Section>
        ...
        <Section title="General Clinical Interventions" icon={<CheckCircle className="w-4 h-4 text-purple-500" />}>
          <div className="space-y-3">
            {ar.recommendations.recommendations.map((rec, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-3 flex flex-wrap justify-between items-start gap-3">
                ...
                <div className="flex gap-1.5 ml-auto">
                  <button
                    ...
                    className={`text-xs px-3 py-1 rounded-lg font-medium border whitespace-nowrap ${
          ...
          <div className="space-y-2">
            {ar.followup_actions.follow_up_actions && ar.followup_actions.follow_up_actions.map((act, i) => (
              <div key={i} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
                <input type="checkbox" className="mt-1 accent-ey-yellow" readOnly />
                <div>
      ...
      {/* Floating Chatbot UI */}
      {isChatOpen ? (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 sm:w-96 w-full h-full sm:h-auto bg-white sm:rounded-xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden z-50">
          <div className="bg-ey-dark px-4 py-3 flex justify-between items-center flex-shrink-0">
            <div className="flex items-center gap-2">
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gray-50 sm:h-80 flex flex-col gap-3">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3 rounded-xl text-sm ${
                  msg.role === 'user'
                    ? 'bg-ey-yellow text-ey-dark rounded-br-none'
            ...
            {isChatLoading && (
              <div className="flex justify-start">
                <div className="bg-white border border-gray-200 text-gray-500 rounded-xl rounded-bl-none p-3 shadow-sm">
          ...
          <div className="p-3 border-t border-gray-200 bg-white flex-shrink-0">
            <form onSubmit={handleSendChat} className="flex items-center gap-2">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-ey-yellow"
              />
              <button
                type="submit"
                disabled={!chatInput.trim() || isChatLoading}
                className="bg-ey-dark text-white p-2 rounded-lg hover:bg-black disabled:opacity-50"
              >
        ...
        <button
          ...
          className="fixed bottom-6 right-6 bg-ey-dark text-white p-4 rounded-full shadow-xl hover:bg-black transition-transform hover:scale-105 z-50 flex items-center justify-center"
        >
          <MessageSquare className="w-6 h-6 text-ey-yellow" />
        </button>
      )}
